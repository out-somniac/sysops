...
void wcBuffer_remove_block(wcBuffer *buffer, unsigned int index) {
    if ( index < 0 || index >= buffer->current_blocks) { return; }
    free(buffer->blocks[index]);

    // Moving remaining blocks to fill in the gap
    for(unsigned int i=index;i<buffer->max_blocks-1;++i) {
        swap(&buffer->blocks[i], &buffer->blocks[i+1]);
    }
